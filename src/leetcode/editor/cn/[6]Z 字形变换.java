package leetcode.editor.cn;//将一个给定字符串 s 根据给定的行数 numRows ，以从上往下、从左到右进行 Z 字形排列。
//
// 比如输入字符串为 "PAYPALISHIRING" 行数为 3 时，排列如下： 
//
// 
//P   A   H   N
//A P L S I I G
//Y   I   R 
//
// 之后，你的输出需要从左往右逐行读取，产生出一个新的字符串，比如："PAHNAPLSIIGYIR"。 
//
// 请你实现这个将字符串进行指定行数变换的函数： 
//
// 
//string convert(string s, int numRows); 
//
// 
//
// 示例 1： 
//
// 
//输入：s = "PAYPALISHIRING", numRows = 3
//输出："PAHNAPLSIIGYIR"
// 
//示例 2：
//
// 
//输入：s = "PAYPALISHIRING", numRows = 4
//输出："PINALSIGYAHRPI"
//解释：
//P     I    N
//A   L S  I G
//Y A   H R
//P     I
// 
//
// 示例 3： 
//
// 
//输入：s = "A", numRows = 1
//输出："A"
// 
//
// 
//
// 提示： 
//
// 
// 1 <= s.length <= 1000 
// s 由英文字母（小写和大写）、',' 和 '.' 组成 
// 1 <= numRows <= 1000 
// 
// Related Topics 字符串 
// 👍 1137 👎 0


//leetcode submit region begin(Prohibit modification and deletion)
class Solution6 {
    public String convert(String s, int numRows) {
        char[] chars = s.toCharArray();
        // numRows个StringBuffer，用于保存每行的元素
        StringBuffer[] rows = new StringBuffer[numRows];
        for (int i = 0; i < rows.length; i++) {
            rows[i] = new StringBuffer();
        }

        // ！！！没有考虑只有一行的情况
        if (numRows < 2) return s;

        for (int i = 0; i < s.length(); i++) {
            int yuShu = i % (2 * numRows - 2);
            // 代表在竖着的方向
            if(yuShu < numRows){
                rows[yuShu].append(chars[i]);
            }else{  // 代表在斜着的方向
                rows[(2 * numRows - 2) - yuShu].append(chars[i]);
            }
        }

        StringBuffer res = new StringBuffer();
        for (int i = 0; i < rows.length; i++) {
            res = res.append(rows[i]);
        }
        return String.valueOf(res);
    }

    public static void main(String[] args) {
        String s = "P";
        Solution6 solution6 = new Solution6();
        System.out.println(solution6.convert(s,1));
    }
}
//leetcode submit region end(Prohibit modification and deletion)
